...
savestateFile = "lua/tasbots/InputBruteforce/slide.st1"
...
function doingBadAction(frame)
    return mario.getFloorTriangle().distToMario() > 250
end
...
function doingGoodAction()
    return false --_marioObj.overlapsWith(_goalObj)
end
...
function reward(ai, state, action, totalReward, doingBad, doingGood) -- action of the frame before, not current!
    ...
    --         ai.rewardPrevious(state, dir .. ",A", -1000)
    ...
    -- end
    -- -- printf("action = %s - reward = %.2f", action, n)
    -- return distReward
end