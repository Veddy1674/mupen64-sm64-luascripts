...
printf = function(s, ...)
    print(string.format(s, ...)) -- simple printf, no \n management
end
...
emu.atupdatescreen(function()
    ...
    table.insert(inputs, directionToArray(jget.X, jget.Y))

    f = f + 1
    if f > frames or jget.up then
        avi.stopcapture() -- might say "tried to end AVI capture when none was in progress" even if it's not really true

        local inputsFile = io.open("videos/whomp1/videos/inputs.txt", "w")
        if not inputsFile then error("file error") return end
        for _, input in ipairs(inputs) do
            inputsFile:write(table.concat(input, ",") .. "\n")
        end        
        inputsFile:close()

        local fInSecs = (f-1) / 30 -- float
        local diffReal = (os.time() - realTime)
        printf("A video was made with %d frames, for a total of %dh,%dm,%.2fs simulated time (real time: %dh,%dm,%ds)", frames, fInSecs // 3600, fInSecs // 60, fInSecs, diffReal // 3600, diffReal // 60, diffReal)
